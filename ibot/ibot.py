...
def call_iquery_train_ticket(params, location_from_entity, location_to_entity, time_entity):
    subprocess.call(["iquery", params, location_from_entity, location_to_entity, time_entity])
...
def pattern_train_ticket(query):
    ...
    paramsDict = {'动车': 'd', '高铁': 'g', '快车': 'k', '特快': 't', '直达': 'z'}
    params = ''
    for word in words:
        if word in paramsDict.keys():
            params += paramsDict[word]
            continue
    if params != '':
        params = '-' + params

    # printf(u"参数:{}".format(params))
